...
def win():
    ...
    check5 = Checkbutton(win, text="it资讯", font=('微软雅黑', 8, 'bold'), variable=CheckVar6, onvalue=1, offvalue=0)
    ...
    # 定义执行函数
    def study():
        global app_id_out,app_secret_out,template_id_out,user_out,province_out,city_out,birthday1_out,birthday2_out,love_date_out,Whether_Eng_out,key1_out,astro_out,user2_out,key2_out,key3_out,key4_out,key5_out,key6_out
        # 没有选择任何项目的情况下
        if (CheckVar1.get() == 0 and CheckVar2.get() == 0 and CheckVar3.get() == 0 and CheckVar4.get() == 0 and CheckVar5.get() == 0):
            ...
            s6 = "it资讯" if CheckVar6.get() == 1 else ""
            s = "您选择了%s %s %s %s %s" % (s1, s2, s3 ,s4, s5,s6)
            #设置标签lb2的字体
            lb2.config(text=s)
            if(CheckVar1.get() == 0):
                key1_out="否"
